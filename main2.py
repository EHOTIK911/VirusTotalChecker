...
SCAN_INTERVAL = 15  # Интервал между сканированиями в секундах
LOGO_TEXT = "VirusTotal Scanner"
AUTHOR_TEXT = "Разработчик: Petruchio"
VERSION = "1.0"

class VirusTotalScanner:
    def __init__(self, root):
        self.root = root
        self.root.title("VirusTotal File Scanner by Petruchio")
    ...
    def setup_ui(self):
        ...
        self.scan_btn = Button(control_frame, text="Начать сканирование", 
                              command=self.toggle_scan, width=15, state=tk.DISABLED)
        ...
        self.progress_bar.grid(row=0, column=3, padx=5, sticky=tk.EW)
        ...
        status_bar = Label(self.root, textvariable=self.status_var, relief=tk.SUNKEN, 
    ...
    def select_path(self):
        # Создаем диалог выбора файла или папки
        choice = messagebox.askquestion("Выбор", "Хотите выбрать файл? (Нет - выбрать папку)")
        
        if choice == 'yes':
            # Выбор файла
            path = filedialog.askopenfilename(title="Выберите файл для сканирования")
            if path:
                self.target_path = path
                self.scan_btn.config(state=tk.NORMAL)
                self.status_var.set("Выбран файл: " + os.path.basename(path))
            ...
            for root_dir, dirs, files in os.walk(path):
                for file in files:
                    ...
                    try:
                        ...
                        # Сохраняем относительный путь для отображения
                        rel_path = os.path.relpath(file_path, path)
                        item = self.tree.insert("", "end", values=(rel_path, file_size, 
                                                                 "В очереди", "0%", "-", "-"))
    ...
    def start_scan(self):
        if not self.files_queue:
            messagebox.showwarning("Предупреждение", "Нет файлов для сканирования!")
            return
        ...
        if os.path.isfile(self.target_path):
            base_path = os.path.dirname(self.target_path)
